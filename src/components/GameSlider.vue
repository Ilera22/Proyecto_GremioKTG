<template>
    <v-container class="pa-4">
      ...
      <v-slide-group v-model="selectedSlide" class="pa-2" show-arrows>
        <v-slide-item v-for="(item, index) in filteredGames" :key="index" :value="index">
          <v-card class="pb-3 ma-2" height="250" width="260" @click="goToArticle(item.link)">
            <v-img :src="item.img" height="140" cover></v-img>
  ...
  export default {
    ...
    async fetchGames() {
      try {
        const response = await axios.get('https://wp.gremioktg.com/wp-json/wp/v2/posts', {
          params: {
            per_page: 10,
            tags: '2', // Verifica que este sea el ID correcto del tag
            _embed: true
          }
        });
  
        console.log('API Response:', response.data); // Añadir esta línea
  
        if (response.data.length === 0) {
          console.warn('No posts found with the specified tag.');
        }
    ...
    cleanText(text) {
      return text.replace(/<\/?[^>]+(>|$)/g, ""); // Eliminar etiquetas HTML
    },
    goToArticle(link) {
